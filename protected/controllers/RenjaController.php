...
'actions'=>array('create','update','adminSkpd','usulanexcel','cetakexcel','cetakpdf','usulanpdf'),
'users'=>array('@'),
),
array('allow', // allow admin user to perform 'admin' and 'delete' actions
'actions'=>array('admin','delete','modalForm','verifikasi'),
'users'=>array('@'),
),
...
*/
public function actionView($id,$id_notif="")
{
	if($id_notif!="")
		AplikasiKomponen::hapus_notif($id_notif);
	$user = Yii::app()->getComponent('user');
	$model = $this->loadModel($id);
	if($model->sumber_usulan=="Bukan Renstra" )
	{
		if($model->status_verifikasi=="Belum Verifikasi")
		{
			$user->setFlash(
				'warning',
				"<strong>Warning!</strong> Kegiatan Renja ini tidak sesuai dengan Renstra, kegiatan ini akan memerlukan verifikasi Bappeda, pastikan anda mengisi catatan penting untuk memudahkan verifikasi data oleh Bappeda."
			);
		}elseif($model->status_verifikasi=="Tidak Diizinkan")
		{
			$user->setFlash(
				'error',
				"<strong>Warning!</strong> Kegiatan Renja ini tidak sesuai dengan Renstra, telah diverifikasi dan tidak diterima oleh Bappeda, <br>dengan alasan : <b>".$model->alasan_tolak_renja."</b>"
			);
		}elseif($model->status_verifikasi=="Diizinkan")
		{
			$user->setFlash(
				'success',
				"Kegiatan Renja ini tidak sesuai dengan Renstra, telah diverifikasi  dan mendapat persetujuan oleh Bappeda"
			);
		}
...
public function actionCetakExcel($tahun,$skpd,$urusan)
{
	...
	header('Content-Disposition: attachment;filename="Data RENJA SKPD.xls"');
...
if(isset($_POST['Renja']))
{
    ...
    $fileName = "{$uploadedFile}"; 
	 $model->foto = $fileName;
	$criteria = new CDbcriteria();
	$criteria->condition = "kegiatan = '".$model->kegiatan."'AND(tahun_perencanaan<='".$model->tahun."' AND tahun_perencanaan+5>='".$model->tahun."')";
	$rensta = Renstra::model()->find($criteria);
	
	if(sizeof($rensta)<=0)
	{
		$model->sumber_usulan = 'Bukan Renstra';
	}
	...
	if($model->save())
	{
		...
		if($model->sumber_usulan =='Bukan Renstra')
		{
			...
			$pemberitahuan->pesan = ucwords(strtolower($model->skpd_->nama_skpd))." menambahkan Renja yang bukan berdasarkan Renstra.";
			$pemberitahuan->link = "index.php?r=renja/view&id=".$model->id;
			$pemberitahuan->status=0;
			$pemberitahuan->save();
		}
...
{
$dataProvider=new CActiveDataProvider('Renja');
