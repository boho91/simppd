...
class RealisasiFisikDanKeuanganDauDpaController extends Controller
{
/**
...
'actions'=>array('admin','delete','AdminSkpd','adminProgram','history','CreateKegiatan'),
'users'=>array('@'),
),
...
public function actionHistory($id,$jenis)
{
	...
	$model=new RealisasiFisikDanKeuanganDauDpa('search');
...
public function actionHistoryKegiatan($id,$jenis)
{
	
	$model=new RealisasiFisikDanKeuanganDauPerkegiatan('search');
	//$model->unsetAttributes();  // clear any default values
	//if(isset($_GET['RealisasiFisikDanKeuanganDauDpa']))
	//$model->attributes=$_GET['RealisasiFisikDanKeuanganDauDpa'];
	if($jenis=="dpa")
	...
	$model->is_perubahan = $is_perubahan;
	$this->renderPartial('historyKegiatan',array(
		'model'=>$model,
		'jenis'=>$jenis
	),false,true);
}
...
{
	$model=new RealisasiFisikDanKeuanganDauDpa;
...
// $this->performAjaxValidation($model);
	...
	if(isset($_POST['RealisasiFisikDanKeuanganDauDpa']))
	{
		$model->attributes=$_POST['RealisasiFisikDanKeuanganDauDpa'];
		if($model->save())
...
public function actionCreateKegiatan($id,$jenis)
{
...
// $this->performAjaxValidation($model);
	...
	//if(RealisasiFisikDanKeuanganDauPerkegiatan::model()->isExistKegiatan($data->skpd,$data->urusan,$data->bidang,$data->program,$data->kegiatan,$data->tahun,$is_perubahan))
	//{
		//$modelEvaluasi = RealisasiFisikDanKeuanganDauPerkegiatan::model()->getByKegiatan($data->skpd,$data->urusan,$data->bidang,$data->program,$data->kegiatan,$data->tahun,$is_perubahan);
	//}else 
	//{
		$modelEvaluasi = new RealisasiFisikDanKeuanganDauPerkegiatan;
		$modelEvaluasi->skpd = $data->skpd;
		$modelEvaluasi->urusan = $data->urusan;
		$modelEvaluasi->bidang = $data->bidang;
		$modelEvaluasi->program = $data->program;
		$modelEvaluasi->kegiatan = $data->kegiatan;
		$modelEvaluasi->tahun = $data->tahun;
		$modelEvaluasi->id_dpa = $id;
		$modelEvaluasi->is_perubahan = $is_perubahan;
		//$modelEvaluasi->save();
	//}
	
	if(isset($_POST['RealisasiFisikDanKeuanganDauPerkegiatan']))
	{
		$modelEvaluasi->attributes=$_POST['RealisasiFisikDanKeuanganDauPerkegiatan'];
		if($modelEvaluasi->save())
			$this->redirect(array('evaluasiKegiatan','skpd'=>$data->skpd,'urusan'=>$data->urusan,'bidang'=>$data->bidang,'program'=>$data->program,'kegiatan'=>$data->kegiatan,'tahun'=>$data->tahun));
	}

	$this->renderPartial('createKegiatan',array(
		'model'=>$modelEvaluasi,
		'jenis'=>$jenis
	));
}
...
{
$dataProvider=new CActiveDataProvider('RealisasiFisikDanKeuanganDauDpa');
...
{
	$model=RealisasiFisikDanKeuanganDauDpa::model()->findByPk($id);
	//if($model===null)
...
{
if(isset($_POST['ajax']) && $_POST['ajax']==='realisasi-fisik-dan-keuangan-dau-dpa-form')
{
...
public function actionAdminSkpd()
{
	...
	$model= Uniondpa::model()->GroupPerCOndition('skpd',Yii::app()->session['tahun_perencanaan']);
...
{
	$model= Uniondpa::model()->GroupPerCOndition('skpd,urusan',Yii::app()->session['tahun_perencanaan'],$skpd);
...
public function actionAdminBidang($skpd,$urusan)
{
	...
	$model= Uniondpa::model()->GroupPerCOndition('skpd,urusan,bidang',Yii::app()->session['tahun_perencanaan'],$skpd,$urusan);
...
{
	$model= Uniondpa::model()->GroupPerCOndition('skpd,urusan,bidang,program',Yii::app()->session['tahun_perencanaan'],$skpd,$urusan,$bidang);
...
{
	$model= Uniondpa::model()->GroupPerCOndition('skpd,urusan,bidang,program,kegiatan',Yii::app()->session['tahun_perencanaan'],$skpd,$urusan,$bidang,$program);
...
public function actionEvaluasiKegiatan($skpd,$urusan,$bidang,$program,$kegiatan)
{
	...
	$modelDpa->jenis_sumber_dana= "Dana Alokasi Umum";
	...
	$modelDpaPerubahan->jenis_sumber_dana= "Dana Alokasi Umum";
