...
public function actionGridUpdate()
{
	if(isset($_POST['pk']))
	{
		if($_POST['name']=="pagu_tahun_n" or $_POST['name']=='pagu_tahun_n_min_1')
		{
...
{
$dataProvider=new CActiveDataProvider('Kua');
...

{
	...
	$criteria->select = 't.*,sum(pagu_tahun_n) as pagu2, sum(pagu_tahun_n_min_1) as pagu1';
	$model = Kua::model()->findAll($criteria);
	Yii::import('ext.phpexcel.XPHPExcel');    
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B5',' BIDANG URUSSAN PEMERINTAH DAERAH');
	$objPHPExcel->getActiveSheet()->mergeCells('C5:D5');
	$objPHPExcel->getActiveSheet()->getStyle("C5")->getAlignment()->setWrapText(true);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C5','PAGU INDIKATIF');
	$objPHPExcel->getActiveSheet()->getStyle("C6")->getAlignment()->setWrapText(true);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C6','TAHUN N-1');
	$objPHPExcel->getActiveSheet()->getStyle("C6")->getAlignment()->setWrapText(true);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D6','TAHUN N');
	...
	foreach($model as $data)
	{
		...
		$criteria_bidang->select = 't.*,sum(pagu_tahun_n) as pagu2, sum(pagu_tahun_n_min_1) as pagu1';
		$criteria_bidang->group='bidang';
		$bidang = Kua::model()->findAll($criteria_bidang);
		foreach($bidang as $data_bidang)
		{
			...
			$criteria_skpd->select = 't.*,sum(pagu_tahun_n) as pagu2, sum(pagu_tahun_n_min_1) as pagu1,SUM(pagu_tahun_n)- SUM(pagu_tahun_n_min_1) as perubahan, (SUM(pagu_tahun_n)- SUM(pagu_tahun_n_min_1)) / SUM(pagu_tahun_n_min_1) * 100  as persen_perubahan';
			$criteria_skpd->group='skpd';
			$kegiatan = Kua::model()->findAll($criteria_skpd);
	...
	//header('Content-Disposition: attachment;filename="Data Plafon Urusan KUA.pdf"');
...
public function actionCetakPlafonperskpdExcel($tahun)
{
	...
	header('Content-Disposition: attachment;filename="Data Plafon per SKPD (KUA).xls"');
	header('Cache-Control: max-age=0');
...
public function actionCetakPlafonperUrusanExcel($tahun)
{
	...
	$criteria = new CDbCriteria();
	$criteria->condition = ' tahun = '.$tahun.'';
	//$criteria->select = 't.*,sum(pagu_tahun_n) as pagu_tahun_n2, sum(pagu_tahun_n_min_1) as pagu_tahun_n_min_12';
	$criteria->group = "urusan";
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D5','PAGU INDIKATIF');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D6','(Rp)');
	$objPHPExcel->getActiveSheet()->getStyle("E5")->getAlignment()->setWrapText(true);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E5','PAGU INDIKATIF N-1');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E6','(Rp)');
	...
	foreach($model as $data)
	{
		...
		$criteria_bidang = new CDbCriteria();
		$criteria_bidang->condition = 'tahun = '.$tahun.' AND urusan='.$data->urusan.'';
		$criteria_bidang->select = 't.*,sum(pagu_tahun_n) as pagu2, sum(pagu_tahun_n_min_1) as pagu1,(SELECT GROUP_CONCAT(DISTINCT(a.nama_skpd) SEPARATOR ";") FROM kua  LEFT JOIN skpd a ON kua.skpd =a.id WHERE tahun = '.$tahun.' AND urusan='.$data->urusan.' GROUP BY bidang) as namaskpd';
		//select GROUP_CONCAT(DISTINCT(skpd)) from kua
	...
	// Redirect output to a clientâ€™s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Data Plafon per Urusan (KUA).xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
	...
	foreach($model as $data)
	{
		...
		foreach($bidang as $data_bidang)
		{
			...
			$kegiatan = Kua::model()->findAll($criteria_skpd);
				...
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue("C".$rows_awal,$data_skpd->skpd_->nama_skpd);
...
{
$model=new Kua('search');
$model->unsetAttributes();  // clear any default values
$model->skpd= $skpd;
if(isset($_GET['Kua']))
$model->attributes=$_GET['Kua'];
