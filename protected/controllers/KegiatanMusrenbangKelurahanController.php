...
class KegiatanMusrenbangKelurahanController extends Controller
{
...
{
	$model=new KegiatanMusrenbangKelurahan;
	$modelKecamatan=new KegiatanMusrenbang;
	
	// Uncomment the following line if AJAX validation is needed
	// $this->performAjaxValidation($model);
	$model->kunci = 0;
	if(isset($_POST['KegiatanMusrenbangKelurahan']))
	{
		$model->attributes=$_POST['KegiatanMusrenbangKelurahan'];
		$modelKecamatan->attributes=$_POST['KegiatanMusrenbangKelurahan'];
		...
		$modelKecamatan->sumber_usulan = "Musrenbang Kelurahan";
...
public function actionFoto($id)
{
	...
	{
		$model->attributes=$_POST['KegiatanMusrenbangKelurahan'];
		$modelFoto->attributes=$_POST['KegiatanMusrenbangKelurahan'];
		$modelFoto->jenis_musrenbang="Musrenbang Kelurahan";
...
public function actionCetakPlafonPdf($tahun,$kd_urusan)
{
	...
	$criteria->group = "kelurahan";

	$model = KegiatanMusrenbangKelurahan::model()->findAll($criteria);
	...
	foreach($model as $data)
	{
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue("C".$rows_awal,$data->kelurahan_->kelurahan);
...
public function actionUsulanPdf(){
	...
	if(isset($_POST['KegiatanMusrenbangKelurahan']))
	{
		...
		$this->redirect(array('cetakusulanpdf','tahun'=>$model->tahun,'kd_urusan'=>$model->kd_urusan,'kelurahan'=>$model->kelurahan));
	}
...
public function actionCetakUsulanPdf($tahun,$kelurahan,$kd_urusan)
{
	Yii::import('ext.phpexcel.XPHPExcel');    
	...
	$criteria = new CDbCriteria();
	$criteria->condition = ' tahun = '.$tahun.' AND kd_urusan='.$kd_urusan.' AND kelurahan='.$kelurahan.'';
	...
	$criteria->select = "t.*";
	$model = KegiatanMusrenbangKelurahan::model()->findAll($criteria);
	$kelurahan = kelurahan::model()->find(array('condition'=>'id = '.$kelurahan.''));
	$urusan = Urusan::model()->find(array('condition'=>'id = '.$kd_urusan.''));
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1','USULAN KEGIATAN MUSRENBANG Kelurahan '.$kelurahan->kelurahan.' TAHUN '.$tahun.'');
	...
	foreach($model as $data)
	{
		...
		if($currentBidang != $tempBidang){
			...
			$totalPaguPerBidang=KegiatanMusrenbangKelurahan::model()->totalPaguPerBidangKelurahan($data->tahun,$data->kelurahan,$data->kd_bidang);
		...
		if($currentProgram != $tempProgram){
			$totalPaguPerProgram=KegiatanMusrenbangKelurahan::model()->totalPaguPerProgramKelurahan($data->tahun,$data->kelurahan,$data->kd_bidang,$data->kd_prog);
...
public function actionCetakPlafonExcel($tahun,$kd_urusan)
{
	...
	$objPHPExcel->getActiveSheet()->mergeCells('C7:C8');
	$objPHPExcel->getActiveSheet()->getStyle("C7")->getAlignment()->setWrapText(true);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C7','KELURAHAN');
	$objPHPExcel->getActiveSheet()->getStyle('D7')->getAlignment()->setWrapText(true);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D7','PAGU TAHUN 1');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D8','(Rp)');
	$objPHPExcel->getActiveSheet()->getStyle('E7')->getAlignment()->setWrapText(true);
	...
	header('Content-Disposition: attachment;filename="Data Plafon Kegiatan Musrenbang Kelurahan.xls"');
	header('Cache-Control: max-age=0');
...
public function actionUsulanExcel()
{
	...
	if(isset($_POST['KegiatanMusrenbangKelurahan']))
	{
		...
		$this->redirect(array('cetakusulanexcel','tahun'=>$model->tahun,'kd_urusan'=>$model->kd_urusan,'kelurahan'=>$model->kelurahan));
	}
...
public function actionCetakUsulanExcel($tahun,$kelurahan,$kd_urusan)
{
	$criteria = new CDbCriteria();
	$criteria->condition = ' tahun = '.$tahun.' AND kd_urusan='.$kd_urusan.' AND kelurahan='.$kelurahan.'';
	//$criteria->select = "t.*,SUM(pagu_tahun_1) AS sumPagu1, SUM(pagu_tahun_2) AS sumPagu2";
	...
	// Redirect output to a clientâ€™s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Data Usulan Kegiatan Musrenbang Kelurahan.xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
...
{
$dataProvider=new CActiveDataProvider('KegiatanMusrenbangKelurahan');
...
public function actionAdmin($skpd)
{
	...
	$model=new KegiatanMusrenbangKelurahan('search');
	$model->unsetAttributes();  // clear any default values
	$model->kd_skpd = $skpd;
	$model->tahun = Yii::app()->session['tahun_perencanaan'];
	if(isset($_GET['KegiatanMusrenbangKelurahan']))
		$model->attributes=$_GET['KegiatanMusrenbangKelurahan'];
...
public function actionAdminSkpd()
{
	...
	$model=new KegiatanMusrenbangKelurahan('searchPerSKpd');
	$model->tahun= Yii::app()->session['tahun_perencanaan'];
	$model->unsetAttributes();  // clear any default values
	if(isset($_GET['KegiatanMusrenbangKelurahan']))
		$model->attributes=$_GET['KegiatanMusrenbangKelurahan'];
...
{
$model=KegiatanMusrenbangKelurahan::model()->findByPk($id);
if($model===null)
