...
class PpasController extends Controller
{
/**
...
'actions'=>array('index','view','CetakPlafonperUrusanExcel','PlafonperurusanExcel'),
'users'=>array('@'),
),
array('allow', // allow authenticated user to perform 'create' and 'update' actions
'actions'=>array('create','update','migrasi','PlafonperProgramPdf','PlafonperProgramExcel','cetakPlafonperProgrampdf','cetakPlafonperProgramExcel','cetakPlafonperUrusanExcel','cetakPlafonperurusanpdf','PlafonperUrusanPdf','PlafonperUrusanexcel'),
'users'=>array('@'),
),
array('allow', // allow admin user to perform 'admin' and 'delete' actions
'actions'=>array('admin','delete','adminSkpd','modalForm','gridUpdate'),
'users'=>array('@'),
),
...
public function actionCetakPlafonperProgramExcel($tahun,$skpd,$urusan)
	{
		...
		$model = Ppas::model()->findAll($criteria);
		...
		foreach($model as $data)
		{
			...
			$rows_awal++;
				...
				$kegiatan = Ppas::model()->findAll($criteria_kegiatan);
...
{
$dataProvider=new CActiveDataProvider('Ppas');
...
{
$model=new Ppas('search');
$model->unsetAttributes();  // clear any default values
if(isset($_GET['Ppas']))
$model->attributes=$_GET['Ppas'];
$model->tahun = Yii::app()->session['tahun_perencanaan'];
$model->skpd = $skpd;
...
public function migrasiPPasSkpd($skpd)
{
	$criteria = new CDbCriteria;
	$criteria->condition = "t.kd_skpd = '".$skpd."' AND t.status_usulan ='Terima' AND t.tahun='".Yii::app()->session['tahun_perencanaan']."' AND a.id IS NULL";
	$criteria->join = "LEFT OUTER JOIN ppas a ON t.id = a.id_musrenbang_kota";
	$models = KegiatanMusrenbangKota::model()->findAll($criteria);
	foreach($models as $model)
	{
		$item[id_musrenbang_kota] = $model->id;
		//$modelKota = new KegiatanMusrenbangKota;
		$item[skpd] = $model->kd_skpd;
		//$modelKota->kd_skpd = $model->kd_skpd;
		$item[urusan] = $model->kd_urusan;
		//$modelKota->kd_urusan = $model->kd_urusan;
		$item[bidang] = $model->kd_bidang;
		//$modelKota->kd_bidang = $model->kd_bidang;
		$item[program] = $model->kd_prog;
		//$modelKota->kd_prog = $model->kd_prog;
		$item[kegiatan] = $model->kd_kegiatan;
	...
	//	$modelKota->sasaran_daerah = $model->sasaran_daerah;
		$item[plafon_anggaran] = $model->pagu_indikatif; 
	...
	if(sizeof($value)>0)
	{
		...
		$command=$connection->createMultipleInsertCommand('ppas', $value);
		$command->execute();
	}
}
public function migrasiKuaSkpd($skpd)
{
	$criteria = new CDbCriteria;
	$criteria->condition = "t.kd_skpd = '".$skpd."' AND t.status_usulan ='Terima' AND t.tahun='".Yii::app()->session['tahun_perencanaan']."' AND a.id IS NULL";
	$criteria->join = "LEFT OUTER JOIN kua a ON t.id = a.id_musrenbang";
	$models = KegiatanMusrenbangKota::model()->findAll($criteria);
	foreach($models as $model)
	{
		
		//$kegiatanTahunLalu = KegiatanMusrenbangKota::model()->find($criteria2);
		$item[id_musrenbang] = $model->id;
		//$modelKota = new KegiatanMusrenbangKota;
		$item[skpd] = $model->kd_skpd;
		//$modelKota->kd_skpd = $model->kd_skpd;
		$item[urusan] = $model->kd_urusan;
		...
		//$item[sasaran_daerah] = $model->sasaran_daerah;
	//	$modelKota->sasaran_daerah = $model->sasaran_daerah;
		$item[pagu_tahun_n] = $model->pagu_indikatif; 
	...
	{
		$connection = Yii::app()->db->getSchema()->getCommandBuilder();
		$command=$connection->createMultipleInsertCommand('kua', $value);
		$command->execute();
	}
}
