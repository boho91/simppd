 ...
 */
class KegiatanMusrenbangKelurahan extends CActiveRecord
{
	/**
	 * @return string the associated database table name
	 */
	public $pagu1,$pagu2,$pagu_tahun_awal,$pagu_tahun_akhir,$sumPagu1,$sumPagu2,$foto;
	...
	public function totalPaguPerBidangKelurahan($tahun,$kelurahan,$bidang)
	{
		$criteria = new CDbCriteria;
		$criteria->condition='tahun="'.$tahun.'" AND kelurahan="'.$kelurahan.'" AND kd_bidang ="'.$bidang.'"';
	...
	public function totalPaguPerProgramKelurahan($tahun,$kelurahan,$bidang,$program)
	{
		$criteria = new CDbCriteria;
		$criteria->condition='tahun="'.$tahun.'" AND kelurahan="'.$kelurahan.'" AND kd_bidang ="'.$bidang.'" AND kd_prog = "'.$program.'"';
		$criteria->group = "kd_bidang";
		$criteria->select = "SUM(pagu_tahun_1) as pagu_tahun_1,SUM(pagu_tahun_2) as pagu_tahun_2";
		$model = $this->find($criteria);
		$model->pagu_tahun_1 = $model->pagu_tahun_1*1;
	...
	{
		return 'kegiatan_musrenbang_kelurahan';
	}
	...
	public function rules()
	{
		...
		return array(
			array('kd_urusan,uraian, kd_bidang, kd_prog, sasaran_daerah,pagu_tahun_1,kd_kegiatan, prioritas_daerah, sasaran_kegiatan', 'required'),
			array('kd_skpd, kd_urusan, kd_bidang,kelurahan,sumber_dana, kd_prog,pagu_tahun_2, tahun, kunci', 'numerical', 'integerOnly'=>true),
			array('created_by,kecamatan,foto, mod_by,uraian,volume,satuan,urutan', 'length', 'max'=>100),
			//array('foto', 'file','types'=>'jpg, gif, png', 'allowEmpty'=>true),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, kd_skpd, kd_urusan, kd_bidang, kd_prog,kd_kegiatan, tahun, kunci,  sasaran_daerah, prioritas_daerah, sasaran_kegiatan, keterangan, created_date, created_by, mod_date, mod_by', 'safe', 'on'=>'search'),
			array('id, kd_skpd, kd_urusan, kd_bidang, kd_prog,kd_kegiatan, tahun, kunci,  sasaran_daerah, prioritas_daerah, sasaran_kegiatan, keterangan, created_date, created_by, mod_date, mod_by', 'safe', 'on'=>'searchPerSKpd'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'foto_'=>array(self::BELONGS_TO, 'Foto', 'foto'),
		);
	}
	...
	public function searchForumSKPD()
	{
		...
		$criteria->compare('pagu_tahun_awal',$this->pagu_tahun_awal,true);
		$criteria->compare('status_forum_skpd',$this->status_forum_skpd,true);
