 ...
 */
class PpasPerubahan extends CActiveRecord
{
	public $pagu1,$plafon_anggaran1,$plafon_anggaran2,$pagu2,$selisih,$selisihTotal,$sumAnggaran,$anggaran;
	...
	public function totalSelisihPagu($tahun)
	{
		...
		$criteria->condition='tahun="'.$tahun.'"';
		$criteria->group = "tahun";
		$criteria->select = "SUM(plafon_anggaran)-SUM(plafon_anggaran_setelah_perubahan) as pagu1";
		$model = $this->find($criteria);
		return $model->pagu1;
	}
	public function totalPaguPerubahan($tahun)
	{
		...
		$criteria->condition='tahun="'.$tahun.'"';
		$criteria->group = "tahun";
		$criteria->select = "SUM(plafon_anggaran_setelah_perubahan) as pagu1";
		$model = $this->find($criteria);
		return $model->pagu1;
	}
	public function afterFind() {
      ...
      //  $this->pagu2 = AplikasiKomponen::uang($this->pagu_tahun_2);
		...
		$this->selisih = AplikasiKomponen::uang($this->plafon_anggaran_setelah_perubahan - $this->plafon_anggaran);
		$this->pagu1 = AplikasiKomponen::uang($this->pagu1);
		$this->pagu2 = AplikasiKomponen::uang($this->pagu2);
		$this->plafon_anggaran1 = AplikasiKomponen::uang($this->plafon_anggaran);
		$this->plafon_anggaran2 = AplikasiKomponen::uang($this->plafon_anggaran_setelah_perubahan);
	...
	public function rules()
	{
		...
		return array(
			array('urusan, bidang, program, plafon_anggaran_setelah_perubahan,kegiatan, skpd', 'required'),
			array('urusan, bidang, program, kegiatan,plafon_anggaran, skpd,tahun, id_musrenbang_kota', 'numerical', 'integerOnly'=>true),
			array('sasaran, target', 'length', 'max'=>20),
			array('created_by, mod_by', 'length', 'max'=>100),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, urusan, bidang, program, kegiatan, sasaran, target,plafon_anggaran_setelah_perubahan, plafon_anggaran, skpd, created_by, created_date, mod_by, mod_date, id_musrenbang_kota', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'urusan_'=>array(self::BELONGS_TO, 'Urusan', 'urusan'),'urusan_'=>array(self::BELONGS_TO, 'Urusan', 'urusan'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'sasaran' => 'Sasaran',
			'target' => 'Target',
			'plafon_anggaran1'=>'Plafon Anggaran Sementara',
			'plafon_anggaran2'=>'Plafon Anggaran Sementara Setelah Perubahan',
			'pagu1'=>'Plafon Anggaran Sementara',
			'pagu2'=>'Plafon Anggaran Sementara Setelah Perubahan',
	...
	public function getPaguPerubahanPerSkpd($skpd,$tahun)
	{
		$criteria = new CDbCriteria;
		$criteria->condition = "skpd ='".$skpd."' AND tahun = '".$tahun."'";
		$criteria->select = "SUM(plafon_anggaran_setelah_perubahan) as plafon_anggaran_setelah_perubahan";
		$criteria->group ="skpd";
		$model = PpasPerubahan::model()->find($criteria);
		if($model->plafon_anggaran_setelah_perubahan=="")
			$model->plafon_anggaran_setelah_perubahan=0;
		return $model->plafon_anggaran_setelah_perubahan;
	}
	public function getPaguPerubahanPerTahun($tahun)
	{
		$criteria = new CDbCriteria;
		$criteria->condition = "tahun = '".$tahun."'";
		$criteria->select = "SUM(plafon_anggaran_setelah_perubahan) as plafon_anggaran_setelah_perubahan";
	...
	public function getPaguPerubahanPerKegiatan($skpd,$kegiatan,$program,$tahun)
	{
		$criteria = new CDbCriteria;
		$criteria->condition = "skpd ='".$skpd."' AND program='".$program."' AND kegiatan = '".$kegiatan."' AND tahun = '".$tahun."'";
		$criteria->select = "SUM(plafon_anggaran_setelah_perubahan) as plafon_anggaran_setelah_perubahan";
	...
	public function searchPerSKpd()
	{
		...
		$criteria->select="t.*,SUM(plafon_anggaran) as pagu1,SUM(plafon_anggaran_setelah_perubahan) as pagu2";
		$criteria->group = "skpd";
		$criteria->compare('id',$this->id,true);
		$criteria->compare('urusan',$this->urusan);
		$criteria->compare('bidang',$this->bidang);
