 ...
 *
 * The followings are the available columns in table 'dpa':
 * @property string $id
 ...
 */
class Dpa extends CActiveRecord
{
	
	public $pagu1,$paguSebelumPerubahan,$paguSetelahPerubahan,$nama_program,$nama_kegiatan,$totalPerUraian,$totalPerSubUraian;
	public $totalPerItem,$alokasiAnggaran,$uraianKegiatan,$harga_satuan_kegiatan,$totalPerKegiatan;
	public function afterFind() {
        parent::afterFind();
		$this->paguSebelumPerubahan = AplikasiKomponen::uang($this->paguSebelumPerubahan);
		$this->paguSetelahPerubahan = AplikasiKomponen::uang($this->paguSetelahPerubahan);
	...
	public function harga_satuan_parent4($id)
	{
		...
		$criteria->join = "LEFT JOIN dpa c ON t.id = c.parent_id LEFT JOIN dpa d ON c.id = d.parent_id  LEFT JOIN dpa e ON d.id = e.parent_id ";
	...
	public function harga_satuan_parent3($id)
	{
		$criteria = new CDbCriteria;
		$criteria->condition = "d.id IS NOT NULL AND t.id = ".$id."";
		$criteria->join = "LEFT JOIN dpa c ON t.id = c.parent_id LEFT JOIN dpa d ON c.id = d.parent_id  LEFT JOIN dpa e ON d.id = e.parent_id  LEFT JOIN dpa f ON e.id = f.parent_id ";
	...
	public function harga_satuan_parent2($id)
	{
		$criteria = new CDbCriteria;
		$criteria->condition = "d.id IS NOT NULL AND t.id = ".$id."";
		$criteria->join = "LEFT JOIN dpa c ON t.id = c.parent_id LEFT JOIN dpa d ON c.id = d.parent_id  LEFT JOIN dpa e ON d.id = e.parent_id  LEFT JOIN dpa f ON e.id = f.parent_id  LEFT JOIN dpa g ON f.id = g.parent_id ";
	...
	public function harga_satuan_parent1($id)
	{
		$criteria = new CDbCriteria;
		$criteria->condition = "d.id IS NOT NULL AND t.id = ".$id."";
		$criteria->join = "LEFT JOIN dpa c ON t.id = c.parent_id LEFT JOIN dpa d ON c.id = d.parent_id  LEFT JOIN dpa e ON d.id = e.parent_id  LEFT JOIN dpa f ON e.id = f.parent_id  LEFT JOIN dpa g ON f.id = g.parent_id   LEFT JOIN dpa h ON g.id = h.parent_id ";
	...
	public function harga_satuan_per_uraian($id)
	{
		$criteria = new CDbCriteria;
		$criteria->condition = "d.id IS NOT NULL AND t.id = ".$id."";
		$criteria->join = "LEFT JOIN dpa c ON t.id = c.parent_id LEFT JOIN dpa d ON c.id = d.parent_id";
	...
	public function beforeSave() {
		if ($this->isNewRecord)
		{
			...
			$model = $this->find($criteria);
			if(sizeof($model)>0)
				$this->jenis_sumber_dana = $model->jenis_sumber_dana;
	...
	public function rules()
	{
		...
		return array(
			...
			array('tahun, urusan, bidang, program, kegiatan, skpd, parent_id, parentCategory, volume', 'numerical', 'integerOnly'=>true),
			array('created_by, mod_by,capaian_program,target_capaian_program,masukan,target_masukan,keluaran,target_keluaran,hasil,target_hasil,sasaran_kegiatan,sumber_dana,lokasi_kegiatan, satuan,level,status_verifikasi', 'length', 'max'=>100),
			array('harga_satuan,sub_uraian,item,volume,satuan,jenis_sumber_dana', 'length', 'max'=>200),
	...
	public function GroupPerCOndition($group)
	{
		...
		$criteria=new CDbCriteria;
		$criteria->condition='t.tahun="'.Yii::app()->session['tahun_perencanaan'].'" AND(t.status_verifikasi<>"Tolak" AND t.jenis_sumber_dana="Dana Alokasi Umum")';
		...
		$criteria->join = "LEFT JOIN (SELECT *,SUM(volume*harga_satuan) AS jumlah_dpa_perubahan FROM dpa_perubahan where status_verifikasi<>'Tolak' AND jenis_sumber_dana='Dana Alokasi Umum' GROUP BY kegiatan,skpd) a ON a.urusan=t.urusan AND a.bidang=t.bidang AND a.program = t.program AND a.kegiatan=t.kegiatan AND a.skpd=t.skpd AND a.tahun=t.tahun";
		//$criteria->join .= " LEFT JOIN (SELECT *,SUM(harga_satuan*volume) as jumlah_dpa_perubahan FROM dpa_perubahan GROUP BY kegiatan,skpd) b ON b.urusan=t.urusan AND b.bidang=t.bidang AND b.program = t.program AND b.kegiatan=t.kegiatan AND b.skpd=t.skpd AND b.tahun=t.tahun";
		$criteria->select="t.*,SUM(t.volume*t.harga_satuan) as paguSebelumPerubahan,a.jumlah_dpa_perubahan as paguSetelahPerubahan";
		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
			'pagination'=>array(
            'pageSize'=>'25'
	 ...
	 * @return dpa the static model class
	 */
