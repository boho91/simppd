...
class PokokPikiranDprdController extends Controller
{
/**
...
'actions'=>array('create','update','modalForm','AdminSkpd','foto','lanjutkan','tidakLanjut','usulanexcel','cetakusulanexcel','usulanpdf','cetakusulanpdf','plafonexcel','cetakplafonexcel','plafonPdf','cetakPlafonPdf'),
'users'=>array('@'),
),
...
public function actionPlafonPdf(){
	$model=new PokokPikiranDprd;
	if(isset($_POST['PokokPikiranDprd']))
	{
		$model->attributes=$_POST['PokokPikiranDprd'];
...
public function actionCetakPlafonPdf($tahun,$kd_urusan){
	...
	$model = PokokPikiranDprd::model()->findAll($criteria);
...
public function actionUsulanPdf(){
	...
	if(isset($_POST['PokokPikiranDprd']))
	{
		...
		$this->redirect(array('cetakusulanpdf','tahun'=>$model->tahun,'kd_urusan'=>$model->kd_urusan));
	}
...
public function actionCetakUsulanPdf($tahun,$kd_urusan){
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1','RUMUSAN USULAN PROGRAM/KEGIATAN');
	$objPHPExcel->getActiveSheet()->getStyle("A1:M1")->applyFromArray($styleHeading);
	$objPHPExcel->getActiveSheet()->mergeCells('A2:M2');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A2','HASIL POKOK-POKOK PIKIRAN DAN VALIDASI');
	$objPHPExcel->getActiveSheet()->getStyle("A2:M2")->applyFromArray($styleHeading);
	$objPHPExcel->getActiveSheet()->mergeCells('A3:M3');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A3','KOTA PEMATANGSIANTAR TAHUN '.$tahun.'');
	$objPHPExcel->getActiveSheet()->getStyle("A3:M3")->applyFromArray($styleHeading);
	$objPHPExcel->getActiveSheet()->getStyle("A4:M6")->applyFromArray($styleHeading);
	...
	foreach($model as $data)
	{
		...
		if($currentBidang != $tempBidang){
			...
			$totalPaguPerBidang=PokokPikiranDprd::model()->totalPaguPerBidang($data->tahun,$data->kd_bidang);
		...
		if($currentProgram != $tempProgram){
			$totalPaguPerProgram=PokokPikiranDprd::model()->totalPaguPerProgram($data->tahun,$data->kd_bidang,$data->kd_prog);
...
public function actionUsulanExcel()
{
	...
	if(isset($_POST['PokokPikiranDprd']))
	{
		...
		$this->redirect(array('cetakusulanexcel','tahun'=>$model->tahun,'kd_urusan'=>$model->kd_urusan));
	}
...
public function actionCetakUsulanExcel($tahun,$kd_urusan)
{
	$criteria = new CDbCriteria();
	$criteria->condition = ' tahun = '.$tahun.' AND kd_urusan='.$kd_urusan.' ';
	...
	// Redirect output to a clientâ€™s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Data Usulan Pokok Pikiran DPRD.xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
...
public function actionCreate()
{
	...
	if(isset($_POST['PokokPikiranDprd']))
	{
		...
		$model->sumber_usulan= "Pokok Pikiran Dprd";
		$uploadedFile=CUploadedFile::getInstance($model,'foto');
		$fileName = "{$uploadedFile}"; 
		 $model->foto = $fileName;
		if($model->save()){
...
{
$dataProvider=new CActiveDataProvider('PokokPikiranDprd');
...
public function actionAdmin($skpd)
{
	...
	$model=new PokokPikiranDprd('search');
	$model->unsetAttributes();  // clear any default values
	$model->kd_skpd = $skpd;
	$model->tahun = Yii::app()->session['tahun_perencanaan'];
	if(isset($_GET['PokokPikiranDprd']))
		$model->attributes=$_GET['PokokPikiranDprd'];
...
public function actionAdminSkpd()
{
	...
	$model=new PokokPikiranDprd('searchPerSKpd');
	$model->tahun= Yii::app()->session['tahun_perencanaan'];
	$model->unsetAttributes();  // clear any default values
	if(isset($_GET['PokokPikiranDprd']))
		$model->attributes=$_GET['PokokPikiranDprd'];
...
{
$model=PokokPikiranDprd::model()->findByPk($id);
if($model===null)
